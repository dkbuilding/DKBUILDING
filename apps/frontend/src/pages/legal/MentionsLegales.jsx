...
const MentionsLegales = () => {
  ...
  const { content, loading, error } = useMarkdownContent('/legal/MentionsLegales.md');
  ...
  const processMarkdownContent = (markdown) => {
    ...
    return sections.map((section) => {
      ...
      const isEditorSection = section.includes('## Éditeur du site');
      const isHostingSection = section.includes('## Hébergement');
      ...
      return (
        <div key={sectionTitle}>
          <div className="content-section mb-12">
            {isEditorSection || isHostingSection ? (
              <div>
  ...
  const markdownComponents = {
    ...
    p: ({ children }) => {
      ...
      if (text.includes('**Email :**') || text.includes('**Téléphone :**') || text.includes('**Adresse :**')) {
        return (
