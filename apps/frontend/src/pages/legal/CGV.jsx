...
const CGV = () => {
  ...
  const { content, loading, error } = useMarkdownContent('/legal/CGV.md');
  ...
  const processMarkdownContent = (markdown) => {
    ...
    return sections.map((section) => {
      ...
      const isInfoSection = section.includes('## Informations générales');
  ...
  const markdownComponents = {
    ...
    h2: ({ children }) => {
      // Extraire le numéro d'article si présent
      const text = String(children);
      const articleMatch = text.match(/Article (\d+)/);
      
      return (
        <h2 className="text-3xl font-foundation-black text-gray-900 mb-6 uppercase">
          {articleMatch ? (
            <>
              Article <span className="text-dk-yellow hover:font-foundation-black">{articleMatch[1]}</span>
              {(() => {
                const rest = text.replace(/Article \d+/, '').trim();
                // Supprimer les tirets et espaces initiaux pour éviter les doublons
                const cleanedRest = rest.replace(/^[\s-]+/, '').trim();
                return cleanedRest && ` - ${cleanedRest}`;
              })()}
            </>
          ) : (
            transformDKBuilding(children)
    ...
    p: ({ children }) => {
      // Détecter si c'est une ligne de contact (contient **Email :**, **Téléphone :**, etc.)
      const text = String(children);
      ...
      if (text.includes('**Email :**') || text.includes('**Téléphone :**') || text.includes('**Adresse :**') || text.includes('**Horaires :**')) {
        return (
          ...
          <div className="container mx-auto px-6 relative z-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-3">
              <Breadcrumb
                ...
                items={[
                  { label: 'Accueil', path: '/' },
                  { label: 'Conditions générales de vente', path: '/legal/cgv' },
                ]}
              />
            </div>
